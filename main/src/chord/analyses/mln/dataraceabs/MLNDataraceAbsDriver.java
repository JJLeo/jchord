...
public class MLNDataraceAbsDriver extends MLNAnalysisDriver {
	...
	protected void runBaseCase(){
		...
		class Formatter {
			...
			String describeQuad(joeq.Compiler.Quad.Quad quad,
			java.util.Map<joeq.Compiler.Quad.Quad, String> quadCache,
			java.util.Map<joeq.Class.jq_Method, String> methodCache) {
				if (quad == null)
				return "unknown quad";
				String cached = quadCache.get(quad);
				if (cached != null)
				return cached;
				joeq.Class.jq_Method method = quad.getMethod();
				String methodInfo = methodCache.get(method);
				if (methodInfo == null) {
					methodInfo = buildMethodDescription(method);
					methodCache.put(method, methodInfo);
				}
				String fileName = method.getDeclaringClass() == null ? "" : method.getDeclaringClass().getSourceFileName();
				String result = methodInfo + " at " + safeFileName(fileName) + ":" + quad.getLineNumber() + " :: " + quad.toString();
				quadCache.put(quad, result);
				return result;
			}
		}
		Formatter formatter = new Formatter();
		String[] orderedDomains = new String[]{"AS", "I", "M", "P", "C", "E", "V", "Z", "F", "K"};
		...
		java.io.File outDir = new java.io.File(Config.outDirName);
		if (!outDir.exists())
		outDir.mkdirs();
		...
		try {
			...
			for (String domName : orderedDomains) {
				ProgramDom<?> dom = domainMap.get(domName);
				if (dom == null)
				continue;
				java.io.File domFile = new java.io.File(outDir, "dom" + domName + "_locations.txt");
				...
				for (int idx = 0; idx < dom.size(); idx++) {
					...
					if ("AS".equals(domName)) {
						if (val == null) {
							desc = "no abstract thread (placeholder)";
						} else {
							chord.util.tuple.object.Pair<?, ?> pair = (chord.util.tuple.object.Pair<?, ?>) val;
							joeq.Compiler.Quad.Quad start = (joeq.Compiler.Quad.Quad) pair.val0;
							joeq.Class.jq_Method rootMeth = (joeq.Class.jq_Method) pair.val1;
							String startDesc = formatter.describeQuad(start, quadDescCache, methodDescCache);
							String rootDesc = formatter.buildMethodDescription(rootMeth);
							desc = "thread started at " + startDesc + " with root method " + rootDesc;
						}
					} else if ("I".equals(domName) || "E".equals(domName)) {
						desc = formatter.describeQuad((joeq.Compiler.Quad.Quad) val, quadDescCache, methodDescCache);
					} else if ("M".equals(domName)) {
						joeq.Class.jq_Method mVal = (joeq.Class.jq_Method) val;
						desc = methodDescCache.get(mVal);
						if (desc == null) {
							desc = formatter.buildMethodDescription(mVal);
							methodDescCache.put(mVal, desc);
						}
					} else if ("P".equals(domName)) {
						if (val == null) {
							desc = "null program point";
						} else if (val instanceof joeq.Compiler.Quad.Quad) {
							desc = "quad " + formatter.describeQuad((joeq.Compiler.Quad.Quad) val, quadDescCache, methodDescCache);
						} else {
							joeq.Compiler.Quad.BasicBlock bb = (joeq.Compiler.Quad.BasicBlock) val;
							joeq.Class.jq_Method bbMeth = bb.getMethod();
							String methDesc = methodDescCache.get(bbMeth);
							if (methDesc == null) {
								methDesc = formatter.buildMethodDescription(bbMeth);
								methodDescCache.put(bbMeth, methDesc);
							}
							desc = "basic block " + bb.getID() + " of " + methDesc;
						}
					} else if ("C".equals(domName)) {
						if (val == null) {
							desc = "epsilon context";
						} else {
							chord.analyses.alias.Ctxt ctxt = (chord.analyses.alias.Ctxt) val;
							...
							if (elems.length == 0) {
								...
								for (int k = 0; k < elems.length; k++) {
									if (k > 0)
									sb.append("; ");
									sb.append(formatter.describeQuad(elems[k], quadDescCache, methodDescCache));
								}
								sb.append("]");
								desc = sb.toString();
							}
						...
						if (val == null) {
							...
							DomV domVLocal = (DomV) dom;
							joeq.Compiler.Quad.RegisterFactory.Register reg = (joeq.Compiler.Quad.RegisterFactory.Register) val;
							joeq.Class.jq_Method owner = domVLocal.getMethod(reg);
							String ownerDesc = methodDescCache.get(owner);
							if (ownerDesc == null) {
								ownerDesc = formatter.buildMethodDescription(owner);
								methodDescCache.put(owner, ownerDesc);
							}
							String varName = owner == null ? "unknown" : owner.getRegName(reg);
							desc = "variable " + varName + " in " + ownerDesc;
						}
						...
						joeq.Class.jq_Field field = (joeq.Class.jq_Field) val;
						if (field == null) {
							desc = "array element synthetic field";
						} else {
							desc = fieldDescCache.get(field);
							if (desc == null) {
								joeq.Class.jq_Class cls = field.getDeclaringClass();
								String fileName = cls == null ? "" : cls.getSourceFileName();
								desc = cls.getName() + "." + field.getName() + " defined at " + formatter.safeFileName(fileName) + ":0";
								fieldDescCache.put(field, desc);
							}
						}
					} else if ("K".equals(domName)) {
						Integer valInt = (Integer) val;
						desc = valInt == null ? "configuration value" : "configuration value " + valInt.intValue();
					} else {
						desc = val == null ? "" : val.toString();
					}
					domWriter.println(idx + ": " + desc);
		...
		java.io.File scriptFile = new java.io.File(outDir, "explain_tuple.py");
		java.io.PrintWriter scriptWriter = null;
		try {
			scriptWriter = new java.io.PrintWriter(new java.io.BufferedWriter(new java.io.FileWriter(scriptFile)));
			scriptWriter.println("#!/usr/bin/env python3");
			scriptWriter.println("import os");
			scriptWriter.println("import sys");
			scriptWriter.println("import re");
			scriptWriter.println();
			scriptWriter.println("DATA_FILE = os.path.join(os.path.dirname(__file__), \"dom_info.tsv\")");
			scriptWriter.println();
			scriptWriter.println("def load_domain_info():");
			scriptWriter.println("    info = {}");
			scriptWriter.println("    try:");
			scriptWriter.println("        with open(DATA_FILE, 'r', encoding='utf-8') as handle:");
			scriptWriter.println("            for line in handle:");
			scriptWriter.println("                line = line.rstrip('\\n')");
			scriptWriter.println("                if not line:");
			scriptWriter.println("                    continue");
			scriptWriter.println("                parts = line.split('\\t', 2)");
			scriptWriter.println("                if len(parts) < 3:");
			scriptWriter.println("                    continue");
			scriptWriter.println("                domain, idx, desc = parts[0], int(parts[1]), parts[2]");
			scriptWriter.println("                info.setdefault(domain, {})[idx] = desc");
			scriptWriter.println("    except FileNotFoundError:");
			scriptWriter.println("        pass");
			scriptWriter.println("    return info");
			scriptWriter.println();
			scriptWriter.println("TUPLE_SCHEMAS = {");
			scriptWriter.println("    'PI': {'domains': ['P', 'I'], 'template': 'program point {0} is exactly the invocation quad {1}'},");
			scriptWriter.println("    'CICM': {'domains': ['C', 'I', 'C', 'M'], 'template': 'in caller context {0}, invocation {1} may reach method {3} under callee context {2}'},");
			scriptWriter.println("    'threadStartI': {'domains': ['I'], 'template': 'invocation {0} calls java.lang.Thread.start()'},");
			scriptWriter.println("    'MPhead': {'domains': ['M', 'P'], 'template': 'method {0} has entry program point {1}'},");
			scriptWriter.println("    'threadACM': {'domains': ['AS', 'C', 'M'], 'template': 'abstract thread {0} starts in context {1} with root method {2}'},");
			scriptWriter.println("    'PP': {'domains': ['P', 'P'], 'template': 'program point {0} has control-flow successor {1}'},");
			scriptWriter.println("    'threadCICM': {'domains': ['C', 'I', 'C', 'M'], 'template': 'thread-start invocation {1} in context {0} reaches thread root method {3} under context {2}'},");
			scriptWriter.println("    'MPtail': {'domains': ['M', 'P'], 'template': 'method {0} exits at program point {1}'},");
			scriptWriter.println("    'CVC': {'domains': ['C', 'V', 'C'], 'template': 'in context {0}, variable {1} may point to object {2}'},");
			scriptWriter.println("    'FC': {'domains': ['F', 'C'], 'template': 'static field {0} may refer to object {1}'},");
			scriptWriter.println("    'CFC': {'domains': ['C', 'F', 'C'], 'template': 'object {0} refers to object {2} via field {1}'},");
			scriptWriter.println("    'MmethArg': {'domains': ['M', 'Z', 'V'], 'template': 'method {0} uses variable {2} as argument position {1}'},");
			scriptWriter.println("    'EV': {'domains': ['E', 'V'], 'template': 'heap event {0} uses variable {1} as its base reference'},");
			scriptWriter.println("    'escE': {'domains': ['E'], 'template': 'heap event {0} is marked as escaping'},");
			scriptWriter.println("    'PE': {'domains': ['P', 'E'], 'template': 'program point {0} corresponds to heap event {1}'},");
			scriptWriter.println("    'EF': {'domains': ['E', 'F'], 'template': 'heap event {0} accesses field {1}'},");
			scriptWriter.println("    'statF': {'domains': ['F'], 'template': 'field {0} is static'},");
			scriptWriter.println("    'excludeSameThread': {'domains': ['K'], 'template': 'exclude-same-thread flag has value {0}'},");
			scriptWriter.println("    'unlockedRaceHext': {'domains': ['AS', 'C', 'E', 'AS', 'C', 'E'], 'template': 'unlocked race candidate between ({0}, {1}, {2}) and ({3}, {4}, {5})'},");
			scriptWriter.println("    'statE': {'domains': ['E'], 'template': 'heap event {0} touches static state'},");
			scriptWriter.println("    'escapingRaceHext': {'domains': ['AS', 'C', 'E', 'AS', 'C', 'E'], 'template': 'escaping race candidate between ({0}, {1}, {2}) and ({3}, {4}, {5})'},");
			scriptWriter.println("    'parallelRaceHext': {'domains': ['AS', 'C', 'E', 'AS', 'C', 'E'], 'template': 'parallel race candidate between ({0}, {1}, {2}) and ({3}, {4}, {5})'},");
			scriptWriter.println("    'datarace': {'domains': ['AS', 'C', 'E', 'AS', 'C', 'E'], 'template': 'confirmed data race between ({0}, {1}, {2}) and ({3}, {4}, {5})'},");
			scriptWriter.println("    'racePairs_cs': {'domains': ['E', 'E'], 'template': 'data race detected between events {0} and {1}'},");
			scriptWriter.println("    'simplePM_cs': {'domains': ['C', 'P', 'C', 'M'], 'template': 'call at {1} in context {0} may enter method {3} with callee context {2}'},");
			scriptWriter.println("    'simplePH_cs': {'domains': ['C', 'P', 'C', 'P'], 'template': 'call at {1} in context {0} reaches callee entry {3} under context {2}'},");
			scriptWriter.println("    'simplePT_cs': {'domains': ['C', 'P', 'C', 'P'], 'template': 'call at {1} in context {0} returns at {3} under context {2}'},");
			scriptWriter.println("    'threadPM_cs': {'domains': ['C', 'P', 'C'], 'template': 'thread start at {1} in context {0} launches thread context {2}'},");
			scriptWriter.println("    'threadPH_cs': {'domains': ['C', 'P', 'C', 'P'], 'template': 'thread start at {1} in context {0} enters thread entry {3} under context {2}'},");
			scriptWriter.println("    'threadAC': {'domains': ['AS', 'C'], 'template': 'abstract thread {0} is runnable in context {1}'},");
			scriptWriter.println("    'threadACH': {'domains': ['AS', 'C', 'P'], 'template': 'abstract thread {0} in context {1} begins at program point {2}'},");
			scriptWriter.println("    'PathEdge_cs': {'domains': ['C', 'P', 'AS', 'AS', 'AS'], 'template': 'in context {0}, dataflow reaches {1} taking current thread {2} with parallel set {3} to {4}'},");
			scriptWriter.println("    'SummEdge_cs': {'domains': ['C', 'P', 'AS', 'AS', 'AS'], 'template': 'summary at {1} in context {0} maps ({2}, {3}) to {4}'},");
			scriptWriter.println("    'mhp_cs': {'domains': ['C', 'P', 'AS', 'AS'], 'template': 'in context {0} at {1}, thread {2} may run with thread {3}'},");
			scriptWriter.println("    'escO': {'domains': ['C'], 'template': 'object {0} escapes its creating thread'},");
			scriptWriter.println("    'CEC': {'domains': ['C', 'E', 'C'], 'template': 'in context {0}, escaping event {1} can access object {2}'},");
			scriptWriter.println("    'mhe_cs': {'domains': ['C', 'E', 'AS', 'AS'], 'template': 'in context {0}, event {1} may run in thread {2} while thread {3} is parallel'},");
			scriptWriter.println("}");
			scriptWriter.println();
			scriptWriter.println("def explain(tuple_text, dom_data):");
			scriptWriter.println("    match = re.fullmatch(r'\s*([A-Za-z_][A-Za-z0-9_]*)\((.*)\)\s*', tuple_text)");
			scriptWriter.println("    if not match:");
			scriptWriter.println("        raise ValueError(f'Malformed tuple: {tuple_text}')");
			scriptWriter.println("    name = match.group(1)");
			scriptWriter.println("    if name not in TUPLE_SCHEMAS:");
			scriptWriter.println("        raise KeyError(f'Unknown tuple predicate: {name}')");
			scriptWriter.println("    args_part = match.group(2).strip()");
			scriptWriter.println("    args = [] if not args_part else [a.strip() for a in args_part.split(',')]");
			scriptWriter.println("    schema = TUPLE_SCHEMAS[name]");
			scriptWriter.println("    if len(args) != len(schema['domains']):");
			scriptWriter.println("        raise ValueError(f'Expected {len(schema['domains'])} arguments for {name}')");
			scriptWriter.println("    resolved = []");
			scriptWriter.println("    for dom_name, arg in zip(schema['domains'], args):");
			scriptWriter.println("        idx = int(arg)");
			scriptWriter.println("        desc = dom_data.get(dom_name, {}).get(idx, f'{dom_name}{idx}')");
			scriptWriter.println("        resolved.append(desc)");
			scriptWriter.println("    return schema['template'].format(*resolved)");
			scriptWriter.println();
			scriptWriter.println("def main():");
			scriptWriter.println("    if len(sys.argv) != 2:");
			scriptWriter.println("        print(f'Usage: {os.path.basename(sys.argv[0])} PREDICATE(args)', file=sys.stderr)");
			scriptWriter.println("        sys.exit(1)");
			scriptWriter.println("    dom_data = load_domain_info()");
			scriptWriter.println("    if not dom_data:");
			scriptWriter.println("        print('Domain information is unavailable; run the analysis first.', file=sys.stderr)");
			scriptWriter.println("        sys.exit(1)");
			scriptWriter.println("    try:");
			scriptWriter.println("        explanation = explain(sys.argv[1], dom_data)");
			scriptWriter.println("    except Exception as exc:");
			scriptWriter.println("        print(str(exc), file=sys.stderr)");
			scriptWriter.println("        sys.exit(1)");
			scriptWriter.println("    print(explanation)");
			scriptWriter.println();
			scriptWriter.println("if __name__ == '__main__':");
			scriptWriter.println("    main()");
		} catch (Exception e) {
			throw new RuntimeException(e);
		} finally {
